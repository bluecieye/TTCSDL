...
namespace TTCSDL_NHOM7.GUIs
{
    public partial class CHONPHIM : Form
    {
        ...
        //-----------------------------------------theo ten -----
        private void TimKiemPhimTheoTen(string tenPhim, DateTime ngay)
        {
            try
            {
                lbphim.Text = "Đang tìm kiếm...";
                flowLayoutPanelPhim.Controls.Clear();

                // Tìm kiếm trực tiếp từ database
                dtPhim = DuLieuDAO.LayPhimDangChieu_TimKiem(ngay, tenPhim);

                if (dtPhim != null && dtPhim.Rows.Count > 0)
                {
                    lbphim.Text = $"Tìm thấy {dtPhim.Rows.Count} phim với từ khóa '{tenPhim}' ngày {ngay:dd/MM/yyyy}";
                ...
                {
                    lbphim.Text = $"Không tìm thấy phim '{tenPhim}' ngày {ngay:dd/MM/yyyy}";
                    Label lbl = new Label
                    {
                        Text = $"Không có phim nào có tên chứa '{tenPhim}'\nchiếu ngày {ngay:dd/MM/yyyy}",
        ...
        private void btntenphim_Click(object sender, EventArgs e)
        {
            ...
            {
                TimKiemPhimTheoTen(txttimten.Text.Trim(), dtpNgayChieu.Value);
            }
        }
    }
}